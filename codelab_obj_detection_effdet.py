    ...
    def display(preds, imgs,content,effdet):
        for i in range(len(imgs)):
            ...
            for j in range(len(preds[i]['rois'])):
                ...
                if obj == content:
                    effdet.send_message_to_scratch((x1+x2)*0.5*0.625-200)    #发送指定类别的识别框位置到scratch
...
class EIMNode(AdapterNode):
    ...
    def extension_message_handle(self, topic, payload):
        ...
        if type(content) == str and content in ['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light',
                'fire hydrant', '', 'stop sign', 'parking meter', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep',
                'cow', 'elephant', 'bear', 'zebra', 'giraffe', '', 'backpack', 'umbrella', '', '', 'handbag', 'tie',
                'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball', 'kite', 'baseball bat', 'baseball glove',
                'skateboard', 'surfboard', 'tennis racket', 'bottle', '', 'wine glass', 'cup', 'fork', 'knife', 'spoon',
            ...
            t= Thread(target=effdet_detection, args=(content,self,))
            t.start()
            t.join()
